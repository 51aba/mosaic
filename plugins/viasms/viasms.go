...
var configs_map = map[string]string {
  "api_url": "https://www.viasms.cz/lead/api/v1/",
  "login": "",
  "password": "",
}
...
func set_response_data(code int, pPluginData map[string]any, ret map[string]any, command string) (result bool) {
  ...
  if _, eok := ret["Error"]; eok {
    log.Printf("%v%v SET_RESPONSE_DATA: REJECTED: RET: %v%v", RED, pPluginData["plugin_log"], ret, NC)
  ...
  if nil == app_map || nil == app_map["Id"] {
    log.Printf("%v%v SET_RESPONSE_DATA: APPLICATION_ISNULL_ERROR: RET: %v%v", RED, pPluginData["plugin_log"], ret, NC)
  ...
  if "Returning customer, accepted" == msg {
    ...
    log.Printf("%v%v SET_RESPONSE_DATA: DUPLICATE: %v%v", RED, pPluginData["plugin_log"], msg, NC)
