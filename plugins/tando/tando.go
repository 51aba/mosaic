...
const codename string = "TANDO"

const (
  min_amount int = 1000
  mid_amount int = 30000
  max_amount int = 50000
  period_max int = 30
)

var configs_map = map[string]string {
  "api_url": "https://tando-cz.creditonline.eu/",
  "login": "",
  "password": "",
  "broker_id": "",
  "api_key": "",
...
func register_lead(pPluginData map[string]any, config map[string]string) (result bool) {
  ...
  log.Printf("%s REGISTER_LEAD: TANDO_TYPE: %v", pPluginData["plugin_log"], pPluginData["tando_type"])

  //-------------------------------------------------------------------------------------------------
  if "c_loan" == pPluginData["tando_type"] {
    pPluginData["config"] = nil
    plugin_postfix = "_" + GetString(pPluginData["tango_type"])
    ...
    config = P_init_named(codename, pPluginData, configs_map, "_" + GetString(pPluginData["tango_type"]))
...
func set_response_data(code int, pPluginData map[string]any, ret map[string]any, command string) (result bool) {
  ...
  if nil != ret["err"] {
    ...
    if "c_loan" == strings.ToLower(GetString(pers_data["addinfo"])) {
      log.Printf("%s SET_RESPONSE_DATA: REGISTRATOR: %v", pPluginData["plugin_log"], ret)

      pPluginData["tando_type"] = "loan"
      result = true
...
func set_response_data_get(code int, pPluginData map[string]any, ret map[string]any, command string) (result bool) {
  ...
  log.Printf("%s SET_RESPONSE_DATA_GET: STATUS: %v [ DATA_MAP: %v ]", pPluginData["plugin_log"], ret["status"], ret_data_map)
